...
namespace rpl{
    ...
    uint8_t Table::insert_row(Row *row) {
        std::unordered_map<std::string, Hash_Header *>::iterator it;
        {
            std::unique_lock<std::mutex> lock(mu);
            it = this->rows.find(row->primary_key);
            if (it == this->rows.end()) {
...
//        }
    ...
    uint8_t Table::get(uint64_t time) {
...
//        if (it==rows._map.end()||it->second->next->event_time<time){
////            sleep(1);
//        }
