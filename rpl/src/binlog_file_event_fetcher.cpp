...
Binlog_file_event_fetcher::Binlog_file_event_fetcher(std::vector<std::string> _files) {
    ...
    buffer_size = 1024*1024*1024*2.1;
    event_pos = 0;
    event_next = BIN_LOG_HEADER_SIZE;
    ringBuffer = new RingBuffer<uint8_t>(buffer_size);
    tmpBuffer = new uint8_t[read_size];
...
//    return std::chrono::duration_cast<std::chrono::microseconds>(duration_since_epoch).count(); // 将时长转换为微秒数
    ...
    //return std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::system_clock::now().time_since_epoch()).count(); // 将时长转换为微秒数
...
int Binlog_file_event_fetcher::fetch_a_event(uint8_t *&buf, int &length) {
    if (ringBuffer->size() <= LOG_EVENT_HEADER_LEN){
        std::cout << "time_read: " << time_read << std::endl;
        return 1;
    }
