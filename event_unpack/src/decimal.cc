...
//    long L;
//    uint32 y, z;
//    long long *bb = nullptr, *bb1, *bd = nullptr, *bd0, *bs = nullptr,
//            *delta = nullptr;
...
//
//    if (!nd0) nd0 = nd;
//    k = nd < __DBL_DIG__ + 1 ? nd : __DBL_DIG__ + 1;
//    rv.d= y;
//    if (k > 9) {
...
//    char buf[460 * sizeof(void *)];
//    double res;
//
//    res = my_strtod_int(str, end, error, buf, sizeof(buf));
//    return (*error == 0) ? res : (res < 0 ? -__DBL_MAX__ : __DBL_MAX__);
//}
...
static int ull2dec(ulonglong from, decimal_t *to) {
  ...
  if (__builtin_expect(intg1 > to->len, false)) {
    intg1 = to->len;
    error = E_DEC_OVERFLOW;
  }
  to->frac = 0;
...
int decimal2ulonglong(const decimal_t *from, ulonglong *to) {
  ...
  for (intg = from->intg; intg > 0; intg -= DIG_PER_DEC1) {
    ulonglong y = x;
    x = x * DIG_BASE + *buf++;
    if (__builtin_expect(y > ((ulonglong)ULLONG_MAX / DIG_BASE) || x < y, false)) {
      *to = ULLONG_MAX;
      return E_DEC_OVERFLOW;
    }
  }
